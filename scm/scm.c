...
{
#ifdef _WIN32
    HANDLE hin = (HANDLE)_get_osfhandle(fileno(inport));
    if ((HANDLE)(-1)==hin) return 0;
    switch (GetFileType(hin)) {
    case FILE_TYPE_DISK: return 0;
    case FILE_TYPE_CHAR: return !0; /* console */
    case FILE_TYPE_PIPE: return !0; /* likely running in emacs buffer, set BUF0 flag so char-ready? works. */
    default: return 0;    /* Unknown type, or error in GetFileType. */
    }
#else
