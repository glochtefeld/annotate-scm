     ...
     int cnt;
{
  ...
  if (INUMP(x)) {
    ...
    if (0==blen) { /* blen zero and fshf zero would lead to an array
                      index of -1 in resds[i - ishf] below. */
      if (INUM0==x) return x;
      else blen = 1;
    }
	...
	4	quotient but returns NULL if division is not exact. */
{
  ...
  if (xlen < ylen)
    ...
    case 0:			/* remainder -- just return x */
      quocell = gcpr[0] = mkbig(xlen, sgn); quods = BDIGITS(quocell);
      do {quods[i] = x[i];} while (++i < xlen);
      return quocell;
    case 1:			/* modulo -- return y-x */
      quocell = gcpr[0] = mkbig(ylen, sgn); quods = BDIGITS(quocell);
      do {
	dds += (long) y[i] - x[i];
    ...
    case 3:			/* round-toward-even */
      ...
      dencell = gcpr[1] = scm_copy_big_ash1(x, xlen, dscl);
  ...
  quocell = gcpr[0] = mkbig(xlen==ylen ? xlen+2 : xlen+1, sgn); quods = BDIGITS(quocell);
  if (xlen==ylen) quods[xlen+1] = 0;
  if (y[ylen-1] < (BIGRAD>>1)) { /* normalize operands */
    dscl = BIGRAD/(y[ylen-1]+1);
    dencell = gcpr[1] = mkbig(ylen, 0); dends = BDIGITS(dencell);
    while(j < ylen) {
  ...
  case 3:			/* round toward even */
    ...
    dencell = gcpr[1] = scm_copy_big_ash1(quods, ylen, dscl);
    dends = BDIGITS(dencell);
    rlen = NUMDIGS(dencell);
