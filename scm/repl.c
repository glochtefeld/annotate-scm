...
{
  DWORD dw, n = 0;
  HANDLE hin = (HANDLE)_get_osfhandle(fileno(f));
  if ((HANDLE)(-1)==hin) return -1;
  ...
  case FILE_TYPE_CHAR: /* console */
    return _kbhit();
  case FILE_TYPE_PIPE:
    if (f->_cnt > 0)        /* Undocumented hack, thanks to http://www.jetdrone.com/post/1/16 */
      return 1;
    if (!PeekNamedPipe(hin, NULL, 0, NULL, &n, NULL)) {
      return -1;
    }
    return n > 0;
  default:
    /* Unknown type, or error in GetFileType. */
    return -1;
  }
}
#else
