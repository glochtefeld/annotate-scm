...
static char rcsid[] =
   "$Id: rgx.c,v 1.21 2023/07/16 13:11:23 shouman Exp $";
     ...
     SCM pattern, flags;
{
  ...
  DEFER_INTS;
  z = gcpr[0] = must_malloc_cell((long)sizeof(regex_info), (SCM)tc16_rgx, s_regex);
...
# endif
  ...
  if (err_msg) {
    ...
    protect = gcpr[0] = makstr(LENGTH(pattern));
    tmppat = CHARS(protect);
