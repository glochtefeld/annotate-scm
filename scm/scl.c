...
{
  volatile SCM gcpr[2];
  SCM t = gcpr[0] = copybig(b, 0);	/* sign of temp doesn't matter */
     ...
     int point;
{
  ...
  if (point >= 0) {
    ...
    {
      volatile SCM gcpr[3];
      SCM quo, num = gcpr[0] = product(bmant, VELTS(pows5)[(long) point]);
      int bex = INUM(scm_intlength(num)) - dbl_mant_dig;

      if (bex > 0) {
	SCM den = gcpr[1] = scm_ash(MAKINUM(1L), MAKINUM(bex));
	quo = gcpr[2] = scm_round_quotient(num, den);
	bigrecy(den);
      }
      else
	quo = gcpr[2] = scm_ash(num, MAKINUM(- bex));
    ...
    return ldexp(num2dbl(bmant,(char *)ARG1,s_str2number) / dpows5[-point], point);
  {
    ...
    SCM num, quo, scl = (-point <= maxpow) ?
      ...
      (gcpr[0] = product(VELTS(pows5)[(long)maxpow], VELTS(pows5)[(long)-point-maxpow]));
    ...
    num = gcpr[1] = scm_ash(bmant, MAKINUM(-bex));
    quo = gcpr[2] = scm_round_quotient(num, scl);
    if (INUM(scm_intlength(quo)) > mantlen) {
      bex++;			/* too many bits of quotient */
      /* protection of arguments here verified necessary for Debian gcc v12.3. */
      quo = gcpr[2] = scm_round_quotient(num, (gcpr[3] = scm_ash(scl, MAKINUM(1L))));
    }
     ...
     register long radix;
{
 ...
 adjust:
  mant[j] = 0;
  manstr = gcpr[1] = resizuve(manstr, MAKINUM(j));
...
# endif
  ...
  case '@': {			/* polar input for complex number */
    ...
    second = gcpr[0] = istr2flo(&str[i], (long)(len-i), radix);
  ...
  second = gcpr[0] = istr2flo(&str[i], (long)((len-i)-1), radix);
  if (IMP(second)) return BOOL_F;
...
#endif
  {
    scm_int i = INUM(x), j = INUM(y);
    int sgn;
#ifdef SCM_LLONG
    /* We have an integer type that can hold the product of scm_int*scm_int. */
    SCM_LLONG k = i * (SCM_LLONG)j;
    if (FIXABLE(k))
      return MAKINUM((scm_int)k);
    sgn = (i < 0) ^ (j < 0);
#else
    /* Use unsigned multiplication, which is required to wrap around
       on overflow (ie give the lower half of the full product.
       Signed multiplication is allowed to do any crazy thing on
       overflow, giving optimizers more freedom. */
    scm_uint k;
    if (i < 0) {
      sgn = 1;
      k = - i;
    ...
    if ((k < MOST_POSITIVE_FIXNUM) && (k/i == j))
      return MAKINUM(sgn ? -(scm_int)k : (scm_int)k);
# ifndef BIGDIG
    /* product = MOST_NEGATIVE_FIXNUM is a corner case.  If we have bignums then
       the bignum code will handle it, otherwise check. */
    if ((k/j == i) && sgn && (k <= -MOST_NEGATIVE_FIXNUM))
      return MAKINUM(-(scm_int)k);
# endif
#endif
#ifdef BIGDIG
  ...
  if (INUMP(z1)) {
	...
	break;
      ...
      if (1L==iz2) {
        /* printf("1: %ld * %ld ->\n", iacc, iz1); */
#ifdef SCM_LLONG
	tmp = (SCM_LLONG)iacc*iz1;
        if ((tmp >= LONG_MIN) && (tmp <= LONG_MAX)) {
          acc = long2num((long)tmp);
          break;
        }
#else
        if (iacc < 0)
        ...
        if (iz1 < 0)
          ...
          tmp = tmp * iz1;
        if ((tmp < LONG_MAX) && ((sgn*tmp)/iacc == iz1)) {
          acc = long2num(sgn*tmp);
          break;
        }
#endif
      overflow:
...
#ifdef SCM_LLONG
	tmp = (SCM_LLONG)iacc*iz1;
        if ((tmp < LONG_MIN) || (tmp > LONG_MAX)) goto overflow;
	iacc = (long)tmp;
#else
        if (iacc < 0)
          sgn = -1, tmp = -iacc;
        ...
        if (iz1 < 0)
          ...
          tmp = tmp * iz1;
        if ((tmp > LONG_MAX) || ((sgn*tmp)/iacc != iz1)) goto overflow;
	iacc = sgn*tmp;
#endif
	iz2 = iz2 - 1L;		/* so jumping to gencase works  */
      ...
      /* printf("3: %ld * %ld ->\n", iz1, iz1); */
#ifdef SCM_LLONG
      tmp = (SCM_LLONG)iz1*iz1;
      if ((tmp < LONG_MIN) || (tmp > LONG_MAX)) goto overflow;
#else
        if (iz1 < 0)
          tmp = (scm_uint)(-iz1) * (-iz1);
        else
          tmp = (scm_uint)iz1 * iz1;
        if ((tmp > LONG_MAX) || ((long)tmp/iz1 != iz1)) goto overflow;
#endif
      iz1 = tmp;
      iz2 >>= 1;
    }
...
/* now able to return unnormalized doubles. */
