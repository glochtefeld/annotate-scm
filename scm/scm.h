...
/*
  scm_int must be a signed integer type at least as wide as pointer.
  long is usually an appropriate type, but for Windows 64-bit builds
  pointers are 64 bits and long is only 32 bits, so scm_int will have
  to be long long.
*/
...
#ifdef _WIN32
# ifdef SCM_WIN_DLL
 /* Had to add extern bit to deal with Mingw gcc 10.2.0, otherwise got errors for multiply defined data. <shouman@comcast.net> */
#  define SCM_DLL_EXPORT extern __declspec(dllexport)
#  define SCM_EXPORT SCM_DLL_EXPORT
# else
#  ifdef DLL   /* I'm afraid this is still not really right. Should
                  accomodate some dlls built in, and some dynamically
                  loadable.  Ok unless one dll depends on another. */
...
SCM_EXPORT void scm_protect_temp2 P((SCM *ptr1, SCM *ptr2));
