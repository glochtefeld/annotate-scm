...
#endif
        if (sizeof(SCM) != sizeof(scm_int)) {
          fixconfig("correct\ntypedf ", "scm_int", 0);
        }
#ifdef SCM_LLONG
        if (sizeof(SCM_LLONG) != 2*sizeof(scm_int))
          fixconfig(remsg, "SCM_LLONG", 0);
#else
# ifdef __STDC_VERSION__  
#  if (__STDC_VERSION__ >= 199901L) /* ISO C99 */
        if (sizeof(long long) == 2*sizeof(scm_int))
          fixconfig(addmsg, "SCM_LLONG long long", 0);
#  endif
# endif
#endif
...
void scm_protect_temp2(ptr1, ptr2)
    SCM *ptr1, *ptr2;
{
  return;
}
