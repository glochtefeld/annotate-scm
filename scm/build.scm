    ...
    (let ((suppressors (build:c-suppress 'dlll platform)))
      ...
      (and (batch:try-chopped-command
	   ...
	   (let ((fnames (map c-> c-files)))
	     (and (batch:try-command
                   parms "gcc" "-shared"
                   "-o" (string-append (car fnames) ".dll")
                   (string-append "-Wl,-out-implib," (car fnames) ".lib,-output-def," (car fnames) ".def")
    ...
    (and (batch:try-command parms
			    ...
			    (must-be-first
			     '("-nostartfiles"
			       ...
			       "/usr/lib/crt0.o")
			     (append objects libs
                                     (list (string-append "-Wl,-out-implib," oname
                                                          ".lib,-output-def," oname ".def")))))
	 oname)))
