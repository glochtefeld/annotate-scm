  ...
  case 1: gencase:		/* Have to loop over all dimensions. */
    {
      ...
      if (ARRAY_NDIM(ra0) >= 5) {
	hp_indv = gcpr[0] = make_uve(ARRAY_NDIM(ra0)+0L, MAKINUM(-32L));
     ...
     SCM ra0, proc, ras;
{
  ...
  volatile SCM gcpr[1];
  if (argc >= 5) {
    heap_ve = gcpr[0] = make_vector(MAKINUM(2*argc), BOOL_F);
    rav = VELTS(heap_ve);
    argv = &(rav[argc]);
  }
  for (k = 0; k < argc; k++) {
...
#endif
  if (i >= 5) {
    hp_av = gcpr[0] = make_vector(MAKINUM(i), BOOL_F);
    av = VELTS(hp_av);
    hp_indv = gcpr[1] = make_uve(i+0L, MAKINUM(-32L));
    indv = (long *)VELTS(hp_indv);
  }
  switch TYP7(ra) {
